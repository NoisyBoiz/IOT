  ...
  return (
    <ScrollView>
      ...
      <Image source={{uri: sharedData?.user?.image?sharedData?.user?.image:'https://cdn.glitch.global/ee29f481-23c3-4bdb-a029-c84ebe6fdfef/avatar.png?v=1732878991705',}}style={{width: 130,height: 130,marginBottom: 20,borderRadius: 180,backgroundColor: COLOR.PRIMARY}} />
        ...
        <Divider/>
        <FiledInfoUser label="Chuyên ngành" value={sharedData?.specialty}/>
        <Divider/>
        <FiledInfoUser label="Số giấy phép" value={sharedData?.licenseNumber}/>
        <Divider/>
        <FiledInfoUser label="Ngày cấp" value={sharedData?.licenseIssueDate?new Date(sharedData?.licenseIssueDate).toLocaleDateString():""}/>
        <Divider/>
        <FiledInfoUser label="Nơi làm việc" value={sharedData?.hospital}/>
        <Divider/>
        <FiledInfoUser label="Kinh nghiệm" value={sharedData?.yearsOfExp}/>
